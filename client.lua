...
Citizen.CreateThread(function()
    while FXCore == nil do
        TriggerEvent('FXCore:GetObject', function(obj)
            FXCore = obj
        end)
        Wait(200)
    end
    while FXCore.Functions.GetPlayerData().job == nil do
        Wait(0)
    end
    PlayerData = FXCore.Functions.GetPlayerData()
    while true do
        local me, sleep, hasjob = GetPlayerPed(-1), 250, true
...
AddEventHandler('esx_ets:start', function(data, jobid)
    -- FXCore.Functions.Notify(tostring(data.vehicles[1]).."  FROM START  "..tostring(data.start[1]).."  ".. tostring(data.start[2])) debug
    working = true
    ...
    while true do
        ...
        if not IsPedInVehicle(GetPlayerPed(-1), truck, false) then
            ...
            if not IsVehicleAttachedToTrailer(truck, trailer) then
                ...
                if not DoesBlipExist(trailerBlip) then
                    trailerBlip = addBlip(data.trailer[1], 479, 5, Strings['trailer'])
        ...
        if distance <= 45.0 and IsPedInVehicle(GetPlayerPed(-1), truck, false) and IsVehicleAttachedToTrailer(truck, trailer) then
            sleep = 0
            ...
            if IsPedInVehicle(GetPlayerPed(-1), truck, false) and IsVehicleAttachedToTrailer(truck, trailer) then
    ...
    while true do
        ...
        if not IsPedInVehicle(GetPlayerPed(-1), truck, false) and distance >= 10.0 then
            text(Strings['get_to_truck'], 250)
            if not DoesBlipExist(truckBlip) then
                truckBlip = addBlip(data.start[1], 477, 5, Strings['truck'])
                SetBlipRoute(truckBlip, true)
            end
            if DoesBlipExist(destinationBlip) then
                RemoveBlip(destinationBlip)
            ...
            if not DoesBlipExist(destinationBlip) then
               -- if HasModelLoaded(trailer) then
            ...
            if distance <= 30.0 then
                ...
                if distance <= 10.0 then
                    if IsPedInVehicle(GetPlayerPed(-1), truck, false) then
    ...
    for i = 0, GetVehicleNumberOfWheels(truck) do
        if IsVehicleTyreBurst(truck, i, false) then
            table.insert(damages['tires'], i)
        end
    end
    for i = 0, 13 do
        if not IsVehicleWindowIntact(truck, i) then
            table.insert(damages['windows'], i)
        end
    end
    for i = 0, GetNumberOfVehicleDoors(truck) do
        if IsVehicleDoorDamaged(truck, i) then
            table.insert(damages['doors'], i)
        end
    end
    DeleteVehicle(truck)
    working = false
...
function MenuLista()
    local Work = MenuV:CreateMenu("Trucking Selector", 'Select the Job', 'topleft', 255, 0, 0, 'size-125')
    Work.Theme = "native"
    Work.Position = "topright"
    Work.Color = {R=255,G=255,B=255}
    MenuV:OpenMenu(Work)

    FXCore.Functions.TriggerCallback("esx_ets:jobMenu", function(menu)
        for k, v in ipairs(menu) do
            local button = Work:AddButton({ icon = '<>', label = v.label, value = v, description = 'Select the Job', select = function(button)
                local select = button.Value
                TriggerServerEvent('esx_ets:startJob', k)
                MenuV:CloseMenu(Work)
            end })
        end
    end)
end
...
loadVehicle = function(vehicle, coords, heading)
    ...
    while not HasModelLoaded(model) do Wait(200) RequestModel(model) end
